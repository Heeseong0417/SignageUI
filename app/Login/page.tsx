...
alert("환영합니다.")
setAuthData(response.data.access_token, response.data.refresh_token, user.userId,response.data.user.user_type);
const authData = getAuthData();
...
<StepTtile title={"로그인"} subtitle={"더정확한더똑똑한, 옥외광고서비스를경험해보세요.."} subtitlecolor={"#000000"}/>
    ...
    :(<></>)}  
      ...
      <button onClick={()=>sendApi()} className={` bg-gradient-to-r rounded-md hover:scale-110 from-[#0F8EFD] to-[#3DD87A] hover:bg-[#0F8EFD] text-white w-[10rem] lg:min-w-[14rem] z-[55] px-[2rem] py-[0.8rem] my-[5%]  font-medium }`}>로그인</button>
      </div>
